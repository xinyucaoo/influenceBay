...
function InfluencerListingsPageContent() {
  ...
  const updateParams = useCallback(
    (updates: Record<string, string>) => {
      ...
      router.push(`/influencer-listings?${params.toString()}`);
    },
    [searchParams, router],
  );
  ...
  useEffect(() => {
    ...
    fetch(`/api/influencer-listings?${params.toString()}`)
      .then((res) => res.json())
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {total > 0
                  ? `${total.toLocaleString()} listings from creators`
                  : "Browse sponsorship opportunities from influencers"}
              </p>
            </div>
        ...
        <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-2 px-4 py-3 sm:px-6 lg:px-8">
          ...
          {(q || niche || pricingType) && (
            <Button
              variant="ghost"
              size="sm"
              onClick={() => router.push("/influencer-listings")}
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {listings.map((listing) => {
                ...
                return (
                  <Link
                    key={listing.id}
                    href={`/influencer-listings/${listing.id}`}
                    className="group"
                  >
                    <Card className="h-full transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5 hover:border-violet-200">
                      <CardHeader className="flex flex-row items-start gap-4">
                        <Avatar className="h-12 w-12 shrink-0 ring-2 ring-violet-100">
                          <AvatarImage
                          ...
                          <AvatarFallback className="bg-violet-100 text-violet-700 text-sm font-semibold">
                            {getInitials(influencer?.user?.name ?? influencer?.handle ?? null)}
                          </AvatarFallback>
                        </Avatar>
                        <div className="min-w-0 flex-1">
                          <CardTitle className="text-base group-hover:text-violet-700 transition-colors line-clamp-2">
                            {listing.title}
                          </CardTitle>
                          <p className="text-sm text-muted-foreground">
                            @{influencer?.handle}
