...
export default async function InfluencerListingsPage() {
  ...
  let profile;
  try {
    profile = await prisma.influencerProfile.findUnique({
      where: { userId: session.user.id },
    });
  } catch (e) {
    console.error("Error fetching influencer profile:", e);
    return (
  ...
  let listings;
  try {
    listings = await prisma.influencerListing.findMany({
    ...
    console.error("Error fetching influencer listings:", e);
    return (
      <div className="container mx-auto px-4 py-16 text-center">
        <div className="mx-auto max-w-md">
          <h1 className="text-2xl font-bold">Something went wrong</h1>
          <p className="mt-3 text-muted-foreground">
            We couldn&apos;t load your listings. Please try again later.
          </p>
          <Button asChild className="mt-6" size="lg">
            <Link href="/dashboard">Back to Dashboard</Link>
          </Button>
      ...
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">
            Manage your sponsorship opportunities for brands. Add descriptions, set your quote, or run auctions.
          </p>
        </div>
        <Button asChild className="gap-2 self-start">
          <Link href="/dashboard/influencer-listings/new">
        ...
        <Card>
          <CardContent className="flex flex-col items-center py-16 text-center">
            ...
            <p className="mt-2 max-w-sm text-muted-foreground">
              Create sponsorship opportunities for brands. Describe your content, set a fixed price or auction it.
            </p>
            <Button asChild className="mt-6" size="lg">
              <Link href="/dashboard/influencer-listings/new">
        ...
        <div className="space-y-4">
          {listings.map((listing) => {
            ...
            return (
              <Link
                key={listing.id}
                href={`/dashboard/influencer-listings/${listing.id}`}
                className="block"
              >
